...
// achieve the cyclic left shift of des[]
// size is the size of des[], num is the shift bits
void left_shift(char des[], int size, int num)
{
...
void substitution(char des[], int size1, const char src[], int size2, const int martix[])
{
...
void conduct_subKey(char primaryKey[])
{
    ...
    for (int i  = 0; i < ROUND_TIMES; i++) {
        ...
        // temp[0:55] = {C, D}
        for(int j = 0; j < 28; j++) temp[j] = C[j];
        for(int j = 0; j < 28; j++) temp[j + 28] = D[j];
...
void F(char result[], char R[], char K[])
{
...
void DES_encryption(char des[], const char src[])
{
	...
	for (int i = 0; i < 32; i++) temp[i + 32] = now_R[i];
